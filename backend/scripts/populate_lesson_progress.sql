-- Script SQL pour peupler la table user_progress avec la progression de chaque leçon
-- Ce script crée une entrée pour chaque leçon de chaque cours inscrit par un étudiant

-- Nettoyer les anciennes données (optionnel - décommentez si nécessaire)
-- DELETE FROM user_progress;

-- Insérer les progressions pour toutes les leçons de tous les cours inscrits
...
-- Vérification : Afficher un résumé des données créées
SELECT 
    u.username,
...
-- Vérification : Compter le nombre total d'entrées créées
SELECT 
